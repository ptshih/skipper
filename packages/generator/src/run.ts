...
function parseArgs(argv: string[]): Args {
  ...
  if (!slug) {
    throw new Error('Usage: run.ts <corridor-slug> [--dry-run] [--preview] [--judge-closers] [--duration=short|standard|long] [--json=<path>]')
  }
...
const estSpokenSec = (script: string): number => script.trim().split(/\s+/).filter(Boolean).length / WORDS_PER_SECOND
