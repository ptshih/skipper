...
// populated. For every stop it snaps the POI onto its corridor's frozen polyline
...
// a re-snap pass if a corridor polyline is ever re-frozen. Pure DB work — no TTS,
// no R2, free to run.
//
//   dotenvx run -f .env.development -- bun packages/generator/src/backfill-trigger-points.ts [slug] [--dry-run]
//
// With no slug, backfills every `ready` tour. With a slug, only that corridor's.

import { and, asc, eq } from 'drizzle-orm'
import { db } from '@skipper/db'
import { corridors, pois, tours, tourStops } from '@skipper/db/schema'
...
async function main() {
  ...
  // Tours to backfill, each with its corridor polyline (joined so we snap against
  // the exact frozen geometry the tour was paced on).
  const tourRows = await db
    ...
    .from(tours)
    .innerJoin(corridors, eq(tours.corridorId, corridors.id))
    .where(slug ? and(eq(tours.status, 'ready'), eq(corridors.slug, slug)) : eq(tours.status, 'ready'))
  ...
  for (const t of tourRows) {
    ...
    console.log(`\n${t.corridorName} · tour ${t.tourId} — ${stops.length} stops`)
