// @skipper/studio — server-side narration generation for the shared-corpus model.
//
// pois (shared FACTS) ──1:1── narrations (the shared telling) is the spine: the corpus
// pipeline is `discover` → `enrich` → `generate`, and every drive SELECTS from that
...
// tours are now DEFERRED; only the corpus generators + their pipeline helpers remain.
