...
export const MODEL_PRICING: Record<string, ModelPricing> = {
  ...
  // Opus 4.6 on Amazon Bedrock — EVERY CLAUDE_MODELS key since 2026-09-17, the live planner included
  // (so the rider-triggered warning above now applies to the `us.` row). Bedrock is partner-billed and
  ...
  // silently.
  // The LIVE id (founder: the US profile). Inference stays in US regions; +10%.
  'us.anthropic.claude-opus-4-6-v1': { inputPerMTok: 5.5, outputPerMTok: 27.5 },
  // The global profile — same model, routed to any region with capacity, list price. Not sent by anything
  // since the `us.` call, but priced because the 2026-09-17 calibration + planner-eval runs (and their
  // saved raw turns) name it, and an unpriced id reads as $0.
