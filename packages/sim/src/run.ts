...
//
// Saved drives can name individual or combined narrations. `loadStops` shares their resolution
// with the sweep CLI: frozen trigger coordinates + sequence, current content and app radius policy.
// --release-audit fails on missing subjects or a route where no clip triggers.
//
// ── --gpx: the DESK DRIVE ────────────────────────────────────────────────────────────────────────────
...
async function main() {
  ...
  const { drive, missing } = resolved
  const stops = resolved.stops.map(s => s.ref)
  ...
  if (process.argv.includes('--release-audit') && !report.stops.some(s => s.fired)) throw new Error('Release audit: no clips triggered')
  const r = report
  console.log('\n' + '='.repeat(78))
  ...
  if (gaps) {
    // Missing audio or geometry removes real clips from the timeline and overstates quiet windows.
    const skipped = missing.length
    if (skipped > 0) {
      console.log('')
      console.log(`⚠ ${skipped} of ${drive.selection.length} selection items could not resolve (missing subjects or invalid geometry; the`)
      console.log('  report is incomplete). Quiet windows would be over-reported — refusing to gate on them.')
    } else {
