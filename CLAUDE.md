...
- **A narration is only live once it has non-null audio.** `narrations` is 1:1 with a poi
  (`narrations_poi_uq`); `audio_url` is NOT NULL at the DB boundary — story + wave today. **Break audio is
  ...
  narrations carry a `facts_hash`; scenic/break/wave carry none and are never fact-stale. A drive can't be
  `ready` until every selected stop resolves to audio (the pipeline enforces; the player also defends).
- **A WAVE is ROAM-ONLY — never a drive stop.** `form='wave'` is the scenic tier's ~15s passing call-out
  (name + kind, no facts, no attribution — Wikidata is CC0), generated by `generate-narrations --wave` off
  `source='wikidata'` pins and routed internally as `stopType:'scenic'` so it reuses the named-scenic
  grounding rules. The wave corpus is ~3× the story corpus, so `loadCorpusForRoute` (`apps/api/src/drives.ts`)
  MUST keep excluding it — without that filter `buildDrive`'s candidate pool goes mostly one-liners. Roam
  picks among simultaneous pins by **distance BAND, then form** (`bandM` in `packages/engine/src/roam.ts`):
  nearer wins, and form only breaks a same-band tie — NOT strict story-over-wave (a wave you're passing
  must beat a story a kilometre off).
...
- **The paid studio pipeline runs from the admin console** (`apps/admin`, behind IAP) or the CLIs in
  ...
  `sweep-orphans` / `refetch-poi`) — **safe-by-default: preview unless `--apply`** (`docs/guides/ops-scripts-sop.md`).
