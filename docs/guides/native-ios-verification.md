...
**Status**: TESTFLIGHT DELIVERED / PHYSICAL VERIFICATION PENDING — build 1.2.0 (28) from `ea815397` shipped 2026-09-17 (root check 322, native 380/380 on the detached checkout, archive/export/inspection clean, symbols uploaded and read back, ASC `VALID` / `IN_BETA_TESTING`; record under `.scratch/releases/native-delivery-ea815397-03/`). It carries the 2026-09-16 planner loop change's client-side neighbours, the glass-bar/composer/bubble layout work and the nine UX-audit fixes. Physical-device acceptance is still not established. Earlier text on this line describes build 27: The `977cd1c5` full release passed root checks (309), 339/339 native tests, archive/manual export/inspection, signed fixture exclusion, in-process EAS symbols upload/readback, Apple validation, and upload (delivery UUID `7acfc478-b11c-4dfc-ba4b-fe3d176891db`). App Store Connect processed build 1.2.0 (27) as `VALID` and `IN_BETA_TESTING` (TestFlight internal testing available). The original pipeline script exited 1 solely due to the local delivery UUID parser on JSON output; original failed record is preserved while GeminiRelease generates a separate reconciliation receipt with fresh ASC. Earlier 333/SE/focused results, the `f1ee2eb3` 338/339 failure, and the `843d8d6c` readiness correction retain their exact source authority. A prior old-source symbols diagnostic passed, distinct from new-source delivery. Subsequent Scenarios A–F verification completed on simulator (`simulator-stable`, 370 tests: 343 unit, 27 UI, 0 failures; xcresult `.scratch/ios/Test-1789293301065.xcresult`). Physical preparation failed because the phone was locked (`kAMDMobileImageMounterDeviceLocked`); external unlock request is pending and no physical app or road-test operation is established. A subsequent adversarial runtime audit (2026-09-13) verified fixes across Storage staging orphan cleanup, Planner owner-boundary creation, Auth reset-link retry, and warm deep-link queuing under driving cover; full native suite completed (380 tests: 350 unit, 30 UI, 0 failed, 0 skipped; xcresult `.scratch/ios/Test-1789324116999.xcresult`) and root checks passed (323 script tests). A subsequent release build from current source remains a separate step. See the [conversion plan](../designs/native-ios-conversion.md).
...
- **Accepted UI test evidence (Run 04):**
  ...
  - Attachments with `.keepAlways` lifetime preserve screenshots; full accessibility hierarchy snapshot retention was added afterward and is present in the final full bundle.
...
### Scenarios A–F verification summary and test boundaries (2026-09-13)

Focused verification of criteria A through F has completed across unit and simulator UI suites, establishing `simulator-stable` status under the reviewer acceptance checklist:

- **Criterion A (Reconnect Model Contracts & Full Suite):** Verified reconnect model contracts (`LibraryFilterTests` 17/17 passed) plus seeded 500 error fallback (`.scratch/ios/Test-lib-filter-1789284245.xcresult` and `.scratch/ios/StabilityReview/reconnect-evidence-audit.md`), confirming offline-to-online connectivity reconnect parity without unkeyed race conditions.
- **Criterion B (Account Lifecycle Dual Flows & Host Proof):** Verified dual account lifecycle flows (password 409/reset + signout, delete OTP) in Run 04 (`results_account_lifecycle_revised_04.xcresult`) with settled receipts (`.scratch/ios/AccountLifecycleQA/receipts/`; backups01–03 retain earlier runs). Host proof verified complete local drive directory erasure (`documents/drives/` empty) and keychain envelope reset (`mock-keychain.json` with `explicitlySignedOut == true`, null active session) prior to same-run online relaunch with anonymous guest mint (`mintRequests == 1`).
- **Criterion C (Normal Driving & Location Gate):** Verified location authorization denied gate routing to `Open Settings` plus precise scripted fixes with simulation OFF through Pull over (`apps/ios/.scratch/ios/CD-final-results.xcresult`), validating searching, first fix, NOW PLAYING, pause/resume, and Pull over with End drive returning to `screen.library` (seek/wrap validated in E1 / simulated driving cycle).
- **Criterion D (Cold Deep Links & Account Gates):** Verified cold signed-in deep link directly to drive detail, signed-out inline account wall presentation (`drive.account-wall`) with zero competing or overlapping sheets, and 3 Detail guard tests (`apps/ios/.scratch/ios/DetailState-results.xcresult`).
- **Criterion E (SE AX5 Accessibility & Essential Controls):** Verified iPhone SE (3rd generation) dynamic type / AX5 accessibility layout (`Skipper-SE-tmp` under `accessibility-extra-extra-extra-large`, `apps/ios/.scratch/ios/SE-v3-results.xcresult`, 4/4 passed) and standard layout (`.scratch/ios/scoped-scroll-retry.xcresult`, 4/4 passed), ensuring departure gate start, transport pause/resume/pull over/forward, wrap ride again/back to drive, and planner composer/proposal make/sign-in remain reachable by scrolling without oscillation or viewport clipping.
- **Criterion F (Final Frozen Full Native + Root & Gap Documentation):** Verified final frozen full native suite (`bun run ios:check`, 370 tests: 343 unit, 27 UI, 0 failures; xcresult `.scratch/ios/Test-1789293301065.xcresult`) and root check (`bun run check` passed: package test suites plus 323 script tests, 0 failed), documented full physical device gaps, and halted simulator execution.

#### Physical Device Gaps (Explicitly Documented — Not Simulator Covered)
Per criterion F, the following operational gaps cannot be proven in simulator and remain documented as known device-only boundaries:
1. **Real-Road GPS Hardware:** Multipath GPS degradation, tunnel loss, cellular cell-tower handoffs, and physical vehicle velocity.
2. **Audio System Interruptions:** System audio focus changes, incoming phone/FaceTime calls, Siri interruptions, and ducking behavior.
3. **Bluetooth & Car Interfaces:** CarPlay integration, Bluetooth A2DP reconnects/disconnects, and automotive dashboard metadata updates.
4. **Lock Screen & Background Transport:** Lock screen `MPNowPlayingInfoCenter` transport controls, remote command center events, and background audio execution.
5. **Screen Reader (VoiceOver):** Hardware VoiceOver navigation, audio ducking during speech synthesis, and rotor actions.
6. **Hardware Secure Enclave / Device Keychain:** Hardware-backed keychain migration, biometric access controls, and device-locked state (`kAMDMobileImageMounterDeviceLocked`).
7. **Populated Legacy Upgrade:** In-place upgrade over installed App Store / TestFlight build containing real legacy AsyncStorage / SQLite database.
8. **Distribution & Telemetry:** Current App Store Connect / TestFlight build 1.2.0 (27) was delivered from prior commit `977cd1c5`. A subsequent release build from current source remains a separate step; live TestFlight PostHog telemetry ingestion remains unverified until deployed.


### Adversarial runtime audit and owner-boundary verification (2026-09-13)

An adversarial runtime audit targeted state isolation, account boundaries, and navigation interruptions across Auth, Storage, Planner, and Driving:
- **Storage staging orphan cleanup:** Added `self.clipStore.cleanStaging()` to `StorageService.init` (`apps/ios/Skipper/Core/Storage/StorageService.swift`), purging partial downloads orphaned across crashes while preserving committed manifests, clips, and clip index (`StorageServiceTests.testInitializationCleansStagingWhilePreservingCommittedFiles`). Constrains architecture to one live `StorageService` instance per root URL.
- **Planner suspended create owner boundary:** In `PlannerViewModel.makeDrive` (`apps/ios/Skipper/Features/Planner/PlannerViewModel.swift`), captured `initiatingUserId = session.user?.id` and verified owner identity on API completion and error catches. If owner changes or signs out in flight, drops completion, suppresses background downloads and navigation, suppresses stale 401 wall events, and resets card state to `.idle` via `restoreIdleAfterOwnerChange(cardId:)` (`ProposalCardTests`). Preserves same-owner `startFresh()` navigation and download to protect spent credits.
- **Auth password reset retry:** In `AuthView.swift` (`apps/ios/Skipper/Features/Auth/AuthView.swift`), explicitly cleared `resetSent = false` on "Forgot password?" selection, resolving a permanent UI dead-end where navigating back from a sent confirmation banner trapped the user on "Check your inbox" (`AdversarialAuditUITests.testPasswordResetRetryAfterBackNavigation`).
- **Warm deep link queuing under driving cover:** In `AppModel.swift` (`apps/ios/Skipper/App/AppModel.swift`), warm deep link arrival (`skipper://drives/<id>`) while driving simulation or active playback is frontmost defers navigation via `private var deferredDriveId`, keeping driving screen frontmost without interruption, and delivering target drive detail after drive ends (`apps/ios/SkipperTests/Networking/FoundationTests.swift` and `apps/ios/SkipperUITests/AdversarialAuditUITests.swift`). Post-End-drive delivery of the `…0003` detail is the evidence for the deep-link fix (retained hierarchy 11:27:27).
- **Audit test runs and evidence boundaries:**
  - *Run 6 (`.scratch/ios/batched-audit-run6.xcresult`)*: 61 cases, 60 passed, whole-process exit 65 (58 unit tests passed; UI reset-retry and auth-cancel passed; deep link failed because `XCUIApplication.open(_:)` relaunches the process).
  - *Run 8 (`.scratch/ios/warm-deeplink-run8.xcresult`)*: Warm `system.open`, detail sheet presented over the driving cover, confirmed MEDIUM product finding, fixed.
  - *Run 9 (`.scratch/ios/scoped-audit-run9.xcresult`)*: 70/70 passed on the interim `playbackPresented`-only shape; superseded for `AppModel`.
  - *Run 10 (`.scratch/ios/scoped-audit-run10.xcresult`, task 6189, 11:26:50–11:28:21)*: 70/70 CLOSED (exit 0): 67 unit (Foundation 9, ProposalCard 25, StorageCancellation 8, StorageService 25) + 3 UI (`testReturningAfterAuthCancel`, `testPasswordResetRetryAfterBackNavigation`, `testDeeplinkArrivingUnderDrivingCover`).
  - *Full native (`task-6210`, `.scratch/ios/Test-1789324116999.xcresult`, closed 11:41:29)*: Exit 0 (380 tests: 350 unit, 30 UI, 0 failed, 0 skipped). Root check (`task-6232`): Exit 0 (workspace packages typechecked, lints green, 323/323 script tests passed across 7 files, 0 failed, 1072 expectations).
- **Known LOW residuals and deferred items:**
  - *Owner change during deferred deep link*: Owner change while a deferred link is pending still switches to Library and may present its entry modal during cover dismissal (retained as an open code-trace follow-up for a future one-line clear before `stopDrive` in `SkipperApp.swift`; static code analysis confirms zero old-owner data exposure or credential leakage; not claimed as runtime-tested).
  - *Active playback defer branch*: The `activePlayback != nil` deferral branch in `AppModel` has no dedicated isolated unit test run.
  - *Rapid start test removed*: `testRapidDoubleStart` removed as smoke-only, not considered double-start proof.
  - *Deferred low findings*: OTP non-empty validation gating on deletion, password error text not cleared on edit typing, and missing accessibility identifiers (`AXID`) deferred as non-blocking.
