...
- ~~`packages/db/seed/{materialize,seed}.ts`~~ (**NB (2026-06-19)** the entire `packages/db/seed/` chain — `seed.ts`, `tour-specs.ts`, `seed/data/*.json`, `poi-overrides.ts`, `speakable.ts`, `materialize.ts` — has been DELETED: authored tours are deferred, route materialization moved to **`@skipper/routing`**, and region/poi-override/speakable bootstrap is now admin-console-curated. The `materialize`/`seed` paths named throughout §5b/§12 no longer exist)
