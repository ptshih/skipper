...
route's bounding box), while roam separately swept the basin into `pois` via `sweep-roam-pois.ts`.
...
1. **Discover a region → the shared corpus.** `sweep-roam-pois.ts` (the existing bbox sweep) is
...
2a. **Generate a tour.** `generate.ts` reads candidates from the corpus
...
Discover region (`sweep_roam_pois --apply`) → author a route (Create tour → `materializeRoute`) →
`generate` the draft → (optionally) `generate_roam` for the corpus. The empty-corpus guard makes
the ordering self-enforcing.

## Deferred / open

- **Lazy facts.** The sweep still fetches STORY extracts eagerly (as before). A region-scale sweep
  could discover identities cheaply and fetch facts only for selected POIs at gen time — the schema
  is ready (`facts_fetched_at` nullable + the TTL read-through). Not done; revisit if a big region's
  eager fetch gets expensive.
...
- **Rename.** `sweep_roam_pois` now feeds tours too; a `discover_region` rename is cosmetic and
  deferred to avoid churning the gen_job kind enum + admin.
