...
**Status**: IMPLEMENTED / TESTFLIGHT DELIVERED — latest build 1.2.0 (28) from `ea815397`, 2026-09-17, `VALID` / `IN_BETA_TESTING`; physical-device acceptance and legacy retirement still pending. The 2026-09-12 delivery of build 27 follows: Native conversion is implemented and committed through `977cd1c5`. Full release verification passed root checks (309) and the native test suite (339/339), archive, manual distribution export, inspection, signed fixture exclusion, in-process EAS symbols upload/readback, and Apple validation/upload (delivery UUID `7acfc478-b11c-4dfc-ba4b-fe3d176891db`). App Store Connect processed build 1.2.0 (27) as valid in beta testing (`IN_BETA_TESTING`). The original pipeline exited 1 solely on a local JSON delivery UUID parsing mismatch; the original record is preserved while GeminiRelease generates a separate reconciliation receipt with fresh ASC. Subsequent Scenarios A–F verification completed on simulator (`simulator-stable`, 370/370 native tests passing: 343 unit, 27 UI, 0 failures; xcresult `.scratch/ios/Test-1789293301065.xcresult`). Physical-device testing remains blocked by a locked phone (`kAMDMobileImageMounterDeviceLocked`), with an external unlock request pending and no physical app or road test established; a subsequent release build from current source remains a separate step. Physical upgrade/driving verification and canonical legacy retirement remain pending.
