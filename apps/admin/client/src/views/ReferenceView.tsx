  ...
  return (
    <div className="space-y-8">
      ...
      <Callout variant="info">
        ...
        Listening Review workspace (automatic audio scoring, exceptions, technical checks) → tune (re-synth / regenerate) → explicitly approve → release to
      ...
      <Section title="Pages">
        <Dl
          ...
          rows={[
            ...
            ['Regions', 'The regions the corpus is keyed to — each with its discovery bbox (the area Discover + Generate sweep), a live POI count, and a Draft / Released status. ⚠ A region IS a bbox — or SEVERAL, separated by ";" (2026-08-06), for a shape one rectangle cannot describe: membership is "inside ANY box", every job sweeps each box separately, and the ground BETWEEN two boxes is NOT in the region (it belongs to whichever region actually covers it). Up to 8 boxes. ⚠ bboxes may overlap — so a place inside two of them belongs to BOTH, and these counts deliberately do not sum to the corpus size. The POIs page shows every region a place is in, which is also the set a region release will publish it from. Discover POIs, Fuse clusters and the Listening Review release workflow launch here — a row’s own Discover / Fuse / Release button, or select multiple rows for a bulk Discover.'],
            ['Listening Review', 'Region-scoped saved sessions show the exact staged publication set, including combined stories, a deterministic reel targeting 12 full clips, and a focused one-at-a-time review card beside a compact queue. Accept & next saves Good with the visible acceptance acknowledgement and advances only after a successful save; Needs work & next moves a clip into a separate queue. Notes are preserved, the acceptance reason is editable, and accepting does not claim the operator listened. Audio loads automatically without autoplay. Full script, scores, sources and operational tools are expandable. The latest saved session opens automatically. Creating a review automatically starts assess_listening_review: a paid audio-and-script judgment capped at $25 per run. Exact clip/context/model/policy results are cached across sessions; retry reuses completed judgments. Clear model passes count as Good by default. The page resumes assessment progress automatically and keeps the selected recording stable during updates. Low, uncertain, failed, outdated-policy, or human-flagged clips need attention; listening to passing clips is optional. Playback, notes, technical decode/duration checks, and replaying a saved operator drive make no model, TTS, or Routes calls. Every clip must pass technical checks; every clip needs a human or model Good verdict, and advisory findings need an acceptance reason. Unsafe script markup, failed grounding, actual clipping, decode failures, and unknown technical failures cannot be waived. Measured tail-volume, loudness, and non-clipping AAC headroom findings remain advisory and require an acceptance reason from the audio judge or reviewer. Approve release review stays disabled while required verdicts, advisory reasons, or technical checks are missing; the page shows how many clips need attention. Approval is explicit and revalidated atomically by the server; playback never approves. Changed audio, geography, membership, endpoints, or corridor evidence makes approval stale. Only Publish approved set releases content permanently. An individual clip needs its own review. Existing live regeneration remains live. Saved evidence accepts active drives owned by the reviewing account or a signed-in app administrator, even when the app and IAP emails differ; attribution retains the reviewing identity and original drive. Importing evidence neither spends a drive credit nor transfers ownership. Invalid feature, road-trigger, or vehicle-access coordinate pairs block release. Initial launches require two distinct vehicle-access locations and saved route evidence; quiet-window/access notes are operator attestations, not a live closure service. Corrections link back to existing tools; paid regeneration still asks for confirmation.'],
            ['POIs', 'The shared POI corpus — sources, enrichment, narration coverage + freshness, and per-POI curation. Discover POIs (top right) sweeps a region’s bbox into the corpus. Quick-filters across the top triage it — “Needs attention” is the combined remediation queue (stale facts, defects, unattributed or drifted clips); “Off-road” lists POIs with no road-snapped anchor in a snapped region (genuine backcountry — they fire off their centroid or never, so they won’t trigger on a drive). Open a POI for its detail sheet: Facts (with Re-fetch facts), Location (speakable anchor, the ROAD CLASS its anchor snapped to, and eligibility), Narration (play the telling; Regenerate / Re-synth / Release), and Corrections (source-labelled Wikipedia and Wikidata fact edits). Choose a source, then add, edit, or retire a correction; the POI determines its source identity. Wikipedia edits require Re-fetch facts, then forced enrichment; Wikidata edits require forced enrichment. Regenerate afterward. Saving an edit itself spends nothing and does not rewrite audio. An amber “minor road” chip on Location means the anchor sits on a residential or unclassified road — real pavement, but usually not one a drive takes, so the stop triggers from a street nobody is on. EXCLUDE (also on Location) hides a place that exists but can’t be told as a stop — a numbered highway, an administrative boundary: it disappears from NEW drives immediately, but audio is KEPT and already-saved drives keep the stop (a drive’s selection is frozen at build, so nobody loses a stop they spent a credit on). Restoring needs no regeneration. ⚠ ONE EXCEPTION, and it is why Exclude and Delete are both REFUSED there: a place named by a group whose FUSED telling is already RELEASED. That clip’s trigger point is derived from its surviving members on every request, so removing one MOVES a live clip inside drives riders have already downloaded, and removing the last one deletes it from them — the one case where the “frozen selection” promise does not hold. Regenerate the group’s telling first, or exclude the whole group. A reason is required and is shown as a banner on every tab of the sheet. ⚠ An excluded place still COSTS MONEY to enrich or narrate — the paid CLIs do not skip it, only the drive build path does — so it shows an “excluded” badge in the corpus table, the spend readout counts it separately, and the “Hide excluded” filter is how you keep it out of a run. Location also shows GROUPING when a place is part of one: the treatment (cluster or district), the title a driver would use, and the members. Grouping is read-only here — it is decided in bulk by the `classify-treatments` CLI, so a hand edit would be overwritten by the next classification. A place in a group whose FUSED telling is live carries an “in a fused clip” chip: it has no clip of its own any more, and it is NOT a generation gap — the group’s single telling names it. Those places are deliberately absent from new drives, because a rider on one block should hear one story, not nine.'],
            ['Places', 'A region’s CURATED real-world DESTINATIONS that feed the planner (separate from the POI/narration corpus). ⚠ ROLES ARE GONE (2026-08-04): every row IS a destination — somewhere a driver would name as a start or a finish — so there is nothing to tag and pruning is a DELETE rather than a checkbox that never stuck. Break pitstops were removed with them: nothing outside this console ever read that flag, and breaks return with live Places data in M3. What each row carries instead is RANK — how likely a visitor is to name the place out loud, 1 being the most, blank sorting last. Rank is editable inline; it comes from the curation draft, orders the planner’s roster, and gates which places can appear as cold-open examples (rank 1–3). It is the model’s judgement, NOT a Google review count — Places policy exempts only `place_id` from its caching rules and the resolve deliberately fetches no rating. Coords are resolved + stored at curation, so the rider’s picker makes zero live Places calls. There is no detail sheet behind a row — the row already carries every field a place has — so CLICKING A ROW answers the one question the table cannot: where it is. The map centers that pin and opens its name bubble, and clicking a pin lights up its row. Curate (interactive — see Run kinds) seeds a region; Add a place is the manual escape hatch. ⚠ THE SCREEN IS A SPLIT WORKSPACE (2026-08-04): the list sits beside a STICKY map rather than under a banner-sized one, so the map keeps answering “where is this” as you scroll a long list — click either side to find it on the other. Search and the rank filter narrow the list; SELECTING rows exposes a bulk REMOVE, which is a hard delete of that many rows from the planner’s allowlist and names them in the confirm. ⚠ A selection SURVIVES a filter change on purpose — if it reaches past what is on screen the bar says “including N hidden by the filter”, so the number you authorise is never a different set than the one deleted. ⚠ ACCESS POINT (the navigation icon on a row, amber when set) is where a car is ROUTED for that place, for the case where the place’s own pin is somewhere a car cannot go — Google Places pins a lake on its water and a beach on its sand, and Google Routes then snaps that to whatever it can reach, which for Spooner Lake was a gated forest track 52 minutes the wrong way. Setting one changes ONLY the Routes request: the map marker, the drive’s title and the coordinates saved into a rider’s drive all keep the real pin, so a rider still sees “Baldwin Beach” on the beach while their car is sent to the public turn-off. It is bounded near the place (2 km) and rejected past that, because an unbounded one is a substitution — it would route a rider somewhere they never asked to go and bill a Routes call to do it. Blank both fields to clear it. ⚠ It is OPERATOR-OWNED: unlike name/coords, neither Curate nor Add-a-place overwrites it — which is the point, since those two DO overwrite a hand-corrected pin. `audit-endpoint-routability --snap` (a CLI) proposes the coordinate for every endpoint whose route Google flags as restricted.'],
      ...
      <Section title="Eval dimensions" subtitle="Scored on eval runs (Generate Narration + Re-score corpus), shown on the Evals page (0–1, higher is better).">
        <Dl
          ...
          rows={[
            ...
            ['charm', 'Persona & delivery quality — advisory (model judge). Opt-in on Re-score corpus.'],
            ['veracity', 'The facts themselves are correct vs the web — advisory (model + Google Search). Opt-in on Re-score corpus.'],
