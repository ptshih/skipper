...
The drive soundtrack is a **shuffled rotation** (`Skipper/Services/Playback/AudioPlayer.swift`): a fresh track
...
| `intro.mp3` / `outro.mp3` | historical, unused drive stings (not bundled in native iOS) | Pixabay Content License | Not used |
...
Sources & Licenses** (`Skipper/Features/Settings/LegalView.swift`, driven by `LegalCredits.swift`) —
...
The player (`Skipper/Services/Playback/AudioPlayer.swift`) shuffles the rotation, plays a fresh track per leg
